...
1. While CICS still owns ACCTDATA reads for Phase 5 programs, Java dual-writes ACCTDATA, TCATBALF and TRANSACT to VSAM **after** its own commit, from the outbox, in the same 2700 -> 2800 -> 2900 order so that a mid-flight failure leaves VSAM in a state the COBOL programs already tolerate. Because TCATBALF and ACCTDATA deltas are additive and outbox delivery is at-least-once, the projection keeps a durable per-(TRAN-ID, target) checkpoint: each of the three VSAM steps is marked applied before the next starts, and a retry skips steps already checkpointed. Recovery after a crash between steps resumes from the first unchecked step; the daily reconciliation additionally compares TCATBALF per key and ACCTDATA per account against the Java ledger and flags any TRAN-ID whose checkpoint is complete but whose delta is missing or doubled.
...
| Data stores | Reads TCATBALF, CARDXREF (+ AIX path), ACCTDATA, DISCGRP, TRANSACT/TRXFL; writes TRANSACT (interest transactions, SYSTRAN GDG), STATEMNT.PS / STATEMNT.HTML. CBACT04C `REWRITE` of ACCTFILE (1050-UPDATE-ACCOUNT, line 356) is the last CICS/batch writer of ACCTDATA; when it migrates, Java becomes sole writer, but the Java -> ACCTDATA dual-write stays on until the last CICS **reader** of ACCTDAT (COPAUS0C, COACCT01, CODATE01 in Phase 6) has migrated or been redirected to the account read API |
...
- `app/app-authorization-ims-db2-mq/cbl/COPAUS0C.cbl` line 870 and `app/app-vsam-mq/cbl/COACCT01.cbl` line 397 (`DATASET(ACCTDAT)` reads from optional modules). `app/cpy/CSUTLDPY.cpy` line 68 (century rule). `app/jcl/POSTTRAN.jcl`, `INTCALC.jcl`, `CREASTMT.JCL`, `READACCT/READCARD/READXREF/READCUST.jcl`, `CBEXPORT.jcl`, `DUSRSECJ.jcl`, `TRANBKP.jcl`, `COMBTRAN.jcl` (EXEC PGM= and DSN= mappings). `README.md` lines 269-326 and 348-351.
