...
├── carddemo-online              — Auth, user management, reports, menu, Spring Security + JWT
...
- `POST /api/reports/generate` triggers `transactionReportJob` via async `JobLauncher`.
- `GET /api/reports/{id}` retrieves report status from `JobExplorer`.
- `GET /api/menu` returns role-based menu items (11 regular + 6 admin from `COMEN02Y`/`COADM02Y`).
- `carddemo-online` depends on `carddemo-batch` for `TransactionReportJobConfig` (imported via `BatchConfig`).
